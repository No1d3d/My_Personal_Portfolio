#ifndef S21_CPP_CONTAINERS_VECTOR_TPP_
#define S21_CPP_CONTAINERS_VECTOR_TPP_
...
void vector<T>::reserve(size_type new_capacity) {
  ...
  if (new_capacity > max_size()) {
    throw std::out_of_range("ReserveError: The size is too large for a new capacity");
  }
...
typename vector<T>::iterator vector<T>::insert(iterator pos,
  ...
  if (idx > size_) {
    throw std::out_of_range(
        "InsertError: The insertion position is out of range of vector "
        "memory");
  }
  if (size_ == capacity_) {
...
#endif //S21_CPP_CONTAINERS_VECTOR_TPP_