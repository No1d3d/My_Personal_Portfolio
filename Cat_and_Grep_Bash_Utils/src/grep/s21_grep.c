...
void s21_grep_operator(int argc, char **argv) {
  if (argc > 1) {
    ...
    for (int x = optionsChoice(argc, argv, &config, &temp); x < argc; x += 1)
  ...
  for (int sym = 0; (sym = getopt(argc, argv, "e:ivclnhsf:o")) != (-1);) {
    switch (sym) {
      ...
      case 'o':
        ...
        break;
      case 'e':
        flagE(config, template, optarg);
        break;
      case 'f':
        flagF(config, template, optarg);
        break;
    }
  }
  if ((config->e || config->f) == 0) {
    templateCompile(template, argv[optind]);
    optind += 1;
  }
  setOptions(config, argc);
  return optind;
}
...
void s21_grep(char *path, options config, char *template) {
  ...
  if (file != NULL) {
    for (char sym = '0'; (sym = getc(file)) != EOF;) {
      ...
      dataOutput(line, &config, template, path);
      free(line);
    }
    printExtraData(config, path);
    fclose(file);
  } else if (config.s == 0) {
  ...
  if (file != NULL) {
    for (char sym = '0'; (sym = getc(file)) != EOF;) {
      ...
      if (!(config->e || config->f))
        config->f = templateCompile(template, line);
      else
        templateAdd(template, line);
      free(line);
    }
    fclose(file);
  } else if (config->s == 0) {
...
void flagE(options *config, char **template, char *optarg) {
  if (!(config->e || config->f))
    config->e = templateCompile(template, optarg);
  else
    templateAdd(template, optarg);
}
...
void dataOutput(char *line, options *config, char *template, char *path) {
  regex_t regex;
...
void printO(regex_t regex, char *line, options config) {
