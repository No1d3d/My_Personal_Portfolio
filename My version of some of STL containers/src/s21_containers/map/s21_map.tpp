...
std::pair<typename map<Key, T>::iterator, bool> map<Key, T>::insert(
  ...
  if (AVLTree<Key, T>::root_ == nullptr) {
    ...
    bool check_insert =
        AVLTree<Key, T>::RecInsert(AVLTree<Key, T>::root_, key, obj);
...
typename map<Key, T>::value_type &map<Key, T>::MapIterator::operator*() {
  ...
  std::pair<const key_type, mapped_type> pr =
      std::make_pair(AVLTree<Key, T>::Iterator::iter_node->key_,
                     AVLTree<Key, T>::Iterator::iter_node->value_);
...
T &map<Key, T>::MapIterator::return_value() {
  ...
  return AVLTree<Key, T>::Iterator::iter_node->value_;
}

template <typename Key, typename T>
typename map<Key, T>::iterator map<Key, T>::find(const Key &key) {
  typename AVLTree<Key, T>::Node *node =
      AVLTree<Key, T>::RecFind(AVLTree<Key, T>::root_, key);
...
void map<Key, T>::erase(map::iterator pos) {
  if (AVLTree<Key, T>::root_ == nullptr || pos.iter_node == nullptr) return;
  AVLTree<Key, T>::root_ =
      AVLTree<Key, T>::RecDelete(AVLTree<Key, T>::root_, (*pos).first);
}
} // namespace s21