...
void glview::projection_processing() {
  projtype = object->condition->projection_type;
  if (projtype == 0) {
...
void glview::line_processing() {
  line_type = object->condition->line_type;
  if (line_type) {
  ...
  if (object->facets_count > 100) {
    glDrawElements(GL_LINES, object->facets_count, GL_UNSIGNED_INT,
                   object->facets_array);
  } else {
    glDrawElements(GL_LINE_LOOP, object->facets_count, GL_UNSIGNED_INT,
                   object->facets_array);
  }
}
...
void glview::vert_processing() {
  ...
  glColor3f(VerticesColor.redF(), VerticesColor.greenF(),
            VerticesColor.blueF());
  vert_type = object->condition->points_type;
  if (vert_type) {
